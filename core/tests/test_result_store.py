...
from core.experiments.result_store import (
    save_csv,
    save_json,
    stable_filename,
...
def test_save_json_with_stable_filename(tmp_path) -> None:
    ...
    out = save_json(
        payload, tmp_path, filename_strategy=stable_filename("metrics.json")
    )

    assert out == tmp_path / "metrics.json"
    assert (
        json.loads(out.read_text(encoding="utf-8"))["summary"]["scenario"] == "simple"
    )
