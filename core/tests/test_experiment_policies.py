...
def make_context(**overrides: object) -> PolicyContext:
    base = {
        ...
        "robot": None,
        "path_length": 3,
        "remaining_path_length": 2,
        "remaining_path": (Position(1, 0), Position(2, 0)),
        "remaining_path_cells": frozenset({Position(1, 0), Position(2, 0)}),
        "planned_cost_by_cell": {},
        "obstacle_cells_changed": frozenset(),
        "cost_cells_changed": frozenset(),
        "world_reinitialized": False,
    }
...
def test_event_based_replan_policy_replans_for_dirty_goal_context() -> None:
    ...
    assert decision.allow_when_not_dirty is False
...
def test_static_once_replan_policy_replans_only_once() -> None:
    ...
    first = policy.decide(make_context(dirty_replan=True))
    second = policy.decide(make_context(tick=1, dirty_replan=True))

    assert first.replan is True
    assert first.allow_when_not_dirty is False
